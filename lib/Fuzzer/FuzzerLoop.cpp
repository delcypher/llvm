...
struct CoverageController {
  static void Reset(const ExternalFunctions &EF) {
  ...
  static void Prepare(const Fuzzer::FuzzingOptions &Options,
                      Fuzzer::Coverage *C, const ExternalFunctions &EF) {
    if (Options.UseCounters) {
...
bool Fuzzer::UpdateMaxCoverage() {
  ...
  bool Res = CoverageController::RecordMax(Options, &MaxCoverage, EF);
...
bool Fuzzer::RunOne(const uint8_t *Data, size_t Size) {
  ...
  CoverageController::ResetCounters(Options, EF);
...
void Fuzzer::ResetCoverage() {
  CoverageController::Reset(EF);
  MaxCoverage.Reset();
  CoverageController::Prepare(Options, &MaxCoverage, EF);
}
